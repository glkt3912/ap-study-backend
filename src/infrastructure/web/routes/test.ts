  ...
  app.post(
    ...
    async (c) => {
      try {
        ...
        const accuracy =
          (morningTest.correctAnswers / morningTest.totalQuestions) * 100;
    ...
    try {
      ...
      return c.json({
        ...
        data: morningTests.map((test) => ({
          ...
          accuracy:
            Math.round((test.correctAnswers / test.totalQuestions) * 1000) / 10,
  ...
  app.get("/morning/stats", async (c) => {
    try {
      ...
      const overallAccuracy = morningStats._sum.totalQuestions
        ? (morningStats._sum.correctAnswers! /
            morningStats._sum.totalQuestions!) *
          100
        : 0;
      ...
      return c.json({
        ...
        data: {
          ...
          averageTimeSpent:
            Math.round((morningStats._avg.timeSpent || 0) * 10) / 10,
          categoryStats: categoryStats.map((stat) => ({
            category: stat.category,
            testCount: stat._count.id,
            averageAccuracy:
              Math.round(
                (stat._avg.correctAnswers! / stat._avg.totalQuestions!) * 1000
              ) / 10,
          })),
        },
      });
    ...
    try {
      ...
      return c.json({
        ...
        data: {
          ...
          averageTimeSpent:
            Math.round((afternoonStats._avg.timeSpent || 0) * 10) / 10,
          categoryStats: categoryStats.map((stat) => ({
            category: stat.category,
            testCount: stat._count.id,
      ...
      try {
        ...
        return c.json({
          ...
          data: morningTests.map((test) => ({
            ...
            correctAnswers: test.correctAnswers,
            accuracy:
              Math.round((test.correctAnswers / test.totalQuestions) * 1000) /
              10,
            timeSpent: test.timeSpent,
            memo: test.memo,
          })),
