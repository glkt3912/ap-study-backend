...
function calculateProgressMetrics(studyPlan: any) {
  ...
  const completedDays = studyPlan.weeks.reduce(
    (acc: number, week: any) => acc + week.days.filter((day: any) => day.completed).length,
    0,
  );
...
function calculatePerformanceMetrics(recentLogs: any[]) {
  const averageUnderstanding =
    recentLogs.length > 0 ? recentLogs.reduce((acc, log) => acc + log.understanding, 0) / recentLogs.length : 0;
